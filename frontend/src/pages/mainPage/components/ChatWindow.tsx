...
const ChatWindow: React.FC<ChatWindowProps> = ({ loggedInUser, selectedUser, onClose }) => {
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                const response = await fetch(`/api/chat/message/${loggedInUser.id}/${selectedUser.id}`, {
                    headers: {
    ...
    return (
        <Box 
            ...
            borderRadius="0.5rem"
        >
            <Button onClick={onClose} mb="4" bg="mingle.lightPurple" color="mingle.darkPurple">Close</Button>
